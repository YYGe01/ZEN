...
def evaluate(args, model, tokenizer, ngram_dict, processor, label_list):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        # sent = [tokenizer.convert_ids_to_tokens(line) for line in input_ids]
        # for cur_sent, cur_tag in map(_single_decode,zip(sent, logits)):
        ...
        for i, label in enumerate(label_ids):
            ...
            for j, m in enumerate(label):
                ...
                labels.append(label_map[logits[i][j]])
            ret.append(labels)
    ...
    # for epoch_num in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0 and loss < min_loss:
            ...
            no_improve = global_step - last_improve
            if no_improve % 20 == 0:
                logger.info("  no_improve = %d", no_improve)
            if no_improve > 1000:
...
def main():
    ...
    parser.add_argument("--predict",
                        action='store_true',
                        help="Whether to run predict on the single sentence.")
    parser.add_argument("--text", help="要进行分割的语句")
    parser.add_argument("--file", help="要进行分割的文件。", default="../datasets/pku_test.utf8")
    parser.add_argument("--out_file", help="分割完成后输出的文件。", default="../datasets/pred_text.utf8")
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0): # 文件预测
        result,ret = evaluate(args, model, tokenizer, ngram_dict, processor, label_list)
        logger.info("\nf1=%s\n" % (str(result["f1"])))
