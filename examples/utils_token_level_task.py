...
class CwsmsraProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "train.tsv")), "train") # _read_tsv 是 DataProcessor 的类方法
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, ngram_dict):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        max_ngram_in_seq_proportion = math.ceil((len(tokens) / max_seq_length) * ngram_dict.max_ngram_in_seq) # 对一个数进行上取整。
