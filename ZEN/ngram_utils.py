    ...
    def __init__(self, ngram_freq_path, tokenizer, max_ngram_in_seq=128):
        ...
        with open(ngram_freq_path, "r", encoding="utf-8") as fin:
            ...
            for i, line in enumerate(fin):
                ...
                # try:
                #     if tokens in self.ngram_to_id_dict:
                #         count +=1
                #         print("wrong",tokens,self.ngram_to_id_dict[tokens],"-----",)
                #         # 长度不一致是因为tokenizer.tokenize进行了wordpiece。
                #     self.ngram_to_id_dict[tokens] = i + 1
                # except:
                #     # print(line)
                #     continue
            #
            # print("total=============",count)
            # print(len(set(self.id_to_ngram_list)))
